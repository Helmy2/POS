...
class DummyDataSeeder(
    ...
    private suspend fun populateDummyUnits(): Map<String, Long> {
        val pieceId = unitDao.insertOrUpdate(UnitEntity(serverId = -1, arName = "قطعة", enName = "Piece", rate = 1.0f, isSynced = true))
        val dozenId = unitDao.insertOrUpdate(UnitEntity(serverId = -2, arName = "دستة", enName = "Dozen", rate = 12.0f, isSynced = true))
        val boxId = unitDao.insertOrUpdate(UnitEntity(serverId = -3, arName = "علبة", enName = "Box", rate = 24.0f, isSynced = true))
        return mapOf("piece" to pieceId, "dozen" to dozenId, "box" to boxId)
    }

    private suspend fun populateDummyStores(): Map<String, Long> {
        val mainStoreId = storeDao.insertOrUpdate(StoreEntity(serverId = -1, arName = "المخزن الرئيسي", enName = "Main Warehouse", type = StoreType.MAIN, isSynced = true))
        val storeAId = storeDao.insertOrUpdate(StoreEntity(serverId = -2, arName = "فرع أ", enName = "Branch A", type = StoreType.SUB, isSynced = true))
        val storeBId = storeDao.insertOrUpdate(StoreEntity(serverId = -3, arName = "فرع ب", enName = "Branch B", type = StoreType.SUB, isSynced = true))
    ...
    private suspend fun populateDummyCategories(): Map<String, Long> {
        val lensesId = categoryDao.insertOrUpdate(CategoryEntity(serverId = -1, arName = "عدسات", enName = "Lenses", isSynced = true))
        val solutionsId = categoryDao.insertOrUpdate(CategoryEntity(serverId = -2, arName = "محاليل", enName = "Solutions", isSynced = true))
        val accessoriesId = categoryDao.insertOrUpdate(CategoryEntity(serverId = -3, arName = "اكسسوارات", enName = "Accessories", isSynced = true))
        return mapOf("lenses" to lensesId, "solutions" to solutionsId, "accessories" to accessoriesId)
    }

    private suspend fun populateDummyProducts(categories: Map<String, Long>, units: Map<String, Long>, stores: Map<String, Long>): Map<String, Long> {
        val product1 = ProductEntity(
            serverId = -1, arName = "عدسات ديزيو الشهرية", enName = "Desio Monthly Lenses",
            categoryId = categories["lenses"], averagePrice = 120.0,
            sellingPrice = 180.0, openingBalanceQuantity = 50.0, storeId = stores["main"], isSynced = true,
            minimumStockUnitId = units["box"], maximumStockUnitId = units["box"],
            minimumStockLevel = 10, maximumStockLevel = 100, firstPeriodData = null
        )
        val prod1Id = productDao.insertOrUpdate(product1)

        val product2 = ProductEntity(
            serverId = -2, arName = "محلول أوبتي-فري", enName = "Opti-Free Solution",
            categoryId = categories["solutions"], averagePrice = 35.5,
            sellingPrice = 55.0, openingBalanceQuantity = 100.0, storeId = stores["main"], isSynced = true,
            minimumStockUnitId = units["piece"], maximumStockUnitId = units["piece"],
            minimumStockLevel = 20, maximumStockLevel = 200, firstPeriodData = "Initial Stock"
        )
        val prod2Id = productDao.insertOrUpdate(product2)

        val product3 = ProductEntity(
            serverId = -3, arName = "حافظة عدسات", enName = "Lens Case",
            categoryId = categories["accessories"], averagePrice = 5.0,
            sellingPrice = 15.0, openingBalanceQuantity = 200.0, storeId = stores["main"], isSynced = true,
            minimumStockUnitId = units["piece"], maximumStockUnitId = units["dozen"],
            minimumStockLevel = 50, maximumStockLevel = 500, firstPeriodData = null
        )
        val prod3Id = productDao.insertOrUpdate(product3)
        return mapOf("desio" to prod1Id, "optiFree" to prod2Id, "lensCase" to prod3Id)
    }

    private suspend fun populateDummyStockTransfers(stores: Map<String, Long>, employees: Map<String, Long>, products: Map<String, Long>, units: Map<String, Long>) {
        val transfer1 = StockTransferEntity(
            serverId = -1, fromStoreId = stores["main"], toStoreId = stores["storeA"],
            initiatedByUserId = employees["admin"], transferDate = System.currentTimeMillis() - 86400000 // 1 day ago
        )
        val transfer1LocalId = stockTransferDao.insertStockTransfer(transfer1)
        stockTransferDao.insertStockTransferItems(listOf(
        ...
        val transfer2 = StockTransferEntity(
            serverId = -2, fromStoreId = stores["main"], toStoreId = stores["storeB"],
            initiatedByUserId = employees["emp1"], transferDate = System.currentTimeMillis()
        )
