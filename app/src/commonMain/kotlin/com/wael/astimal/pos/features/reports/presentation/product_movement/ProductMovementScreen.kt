...
import pos.app.generated.resources.all_products
...
fun ProductMovementRoute(
    ...
    PdfGeneratorEffect(
        ...
        onFinish = { viewModel.processEvent(ProductMovementContract.Event.PdfGenerationFinished) }
    )
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        onClick = { viewModel.processEvent(ProductMovementContract.Event.GeneratePdf) },
...
fun ProductMovementScreen(
    state: ProductMovementContract.State,
    processEvent: (ProductMovementContract.Event) -> Unit,
    ...
    if (showStartDatePicker) {
        DataPicker(
            onDateSelected = { processEvent(ProductMovementContract.Event.SetStartDate(it)) },
            onDismiss = { showStartDatePicker = false }
        )
    }
    if (showEndDatePicker) {
        DataPicker(
            onDateSelected = { processEvent(ProductMovementContract.Event.SetEndDate(it)) },
    ...
    Column(modifier = modifier.padding(16.dp).fillMaxSize()) {
        ...
        Button(
            onClick = { processEvent(ProductMovementContract.Event.ApplyFilters) },
    ...
    Card(elevation = CardDefaults.cardElevation(defaultElevation = 2.dp)) {
        Column(modifier = Modifier.padding(8.dp)) {
            ...
            Row(Modifier.fillMaxWidth().padding(vertical = 8.dp, horizontal = 4.dp)) {
                ...
                Text(
                    String.format("%.2f", group.totalIn),
                    modifier = Modifier.weight(1f),
                ...
                Text(
                    String.format("%.2f", group.totalOut),
                    modifier = Modifier.weight(1f),
                    style = MaterialTheme.typography.bodyMedium,
                    textAlign = TextAlign.End,
                    fontWeight = FontWeight.Bold
            ...
            Row(Modifier.fillMaxWidth().padding(vertical = 8.dp, horizontal = 4.dp)) {
                ...
                Text(
                    String.format("%.2f", group.closingBalance),
                    modifier = Modifier.weight(1.5f),
                    style = MaterialTheme.typography.bodyMedium,
                    textAlign = TextAlign.End,
                    fontWeight = FontWeight.Bold
...
private fun MovementRow(entry: ProductMovementEntry) {
    ...
    Row(
        ...
        Text(
            if (entry.quantityIn > 0) String.format("%.2f", entry.quantityIn) else "",
            modifier = Modifier.weight(1f),
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.End
        )
        Text(
            if (entry.quantityOut > 0) String.format("%.2f", entry.quantityOut) else "",
            modifier = Modifier.weight(1f),
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.End
        )
