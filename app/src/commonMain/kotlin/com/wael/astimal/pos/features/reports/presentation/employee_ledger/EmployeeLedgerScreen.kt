    ...
    Screen(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        onClick = { viewModel.processEvent(EmployeeLedgerReducer.Event.GeneratePdf) },
...
fun EmployeeLedgerScreen(
    state: EmployeeLedgerReducer.State,
    processEvent: (EmployeeLedgerReducer.Event) -> Unit,
    ...
    Column(modifier = modifier.padding(16.dp).fillMaxSize()) {
        ...
        if (state.isLoading) {
            ...
            LedgerTable(entries = state.ledgerEntries, onEntryClick = {
                processEvent(EmployeeLedgerReducer.Event.SelectEntry(it))
            }) {
                FlowRow(
                    ...
                    ExposedDropdownMenu(
                        ...
                        onItemSelected = {
                            processEvent(EmployeeLedgerReducer.Event.SelectEmployee(it?.let {
                    ...
                    AppButton(
                        onClick = { processEvent(EmployeeLedgerReducer.Event.ApplyFilters) },
...
private fun LedgerRow(entry: EmployeeLedgerEntry, onEntryClick: (EmployeeLedgerEntry) -> Unit) {
    ...
    Row(
        ...
        Text(
            if (entry.debit > 0) entry.debit.formate() else "",
            modifier = Modifier.weight(1f),
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.End
        )
        Text(
            if (entry.credit > 0) entry.credit.formate() else "",
            modifier = Modifier.weight(1f),
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.End
        )
