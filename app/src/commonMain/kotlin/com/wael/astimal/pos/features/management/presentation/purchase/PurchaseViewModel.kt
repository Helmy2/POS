...
class PurchaseViewModel(
    ...
    initialState = PurchaseContract.State(
        currentOrderInput = PurchaseContract.EditableOrder(date = Clock.now())
    )
) {
    ...
    val filteredOrdersState: StateFlow<List<Invoice>> = combine(
        state, invoiceRepository.getInvoices().map { list ->
            list.filter { it.invoiceType == InvoiceType.PURCHASE }
        }
    ) { state, allOrders ->
        if (state.orders != allOrders) {
            setState(PurchaseContract.Event.OrdersLoaded(allOrders))
        }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            id?.let {
                invoiceRepository.getInvoiceById(it).onSuccess { invoice ->
                    setState(PurchaseContract.Event.OrderSelected(invoice))
                }.onFailure {
                    it.printStackTrace()
                }
            }
    ...
    private fun loadInitialData() {
        viewModelScope.launch {
            ...
            setState(PurchaseContract.Event.UserLoaded(user))
            combine(
    ...
    private fun saveOrder() {
        viewModelScope.launch {
            ...
            result.onSuccess {
                ...
                setState(PurchaseContract.Event.SaveSucceeded(it))
            }.onFailure {
                it.printStackTrace()
    ...
    private suspend fun generatePdf(
        ...
        val html = htmlReportGenerator.createInvoiceHtml(invoice = invoice,partnerBalance=partnerBalance)

        setState(PurchaseContract.Event.PdfGenerationSuccess(html))
    }
}