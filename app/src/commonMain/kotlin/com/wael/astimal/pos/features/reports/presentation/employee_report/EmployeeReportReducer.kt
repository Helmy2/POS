...
class EmployeeReportReducer :
    ...
    override fun reduce(
        ...
        return when (event) {
            is EmployeeReportContract.Event.ShowInitialData -> {
                previousState.copy(employees = event.employees) to null
            }

            is EmployeeReportContract.Event.SelectEmployee -> {
                previousState.copy(selectedEmployee = event.employee) to null
            }

            is EmployeeReportContract.Event.SetStartDate -> {
                previousState.copy(startDate = event.date) to null
            }

            is EmployeeReportContract.Event.SetEndDate -> {
            ...
            is EmployeeReportContract.Event.ShowTransactions -> {
