...
fun CurrentStockRoute(
    onNavigateBack: () -> Unit,
    viewModel: CurrentStockViewModel = koinInject()
) {
    ...
    PdfGeneratorEffect(
        ...
        onFinish = { viewModel.processEvent(CurrentStockContract.Event.PdfGenerationFinished) }
    )
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        onClick = { viewModel.processEvent(CurrentStockContract.Event.GeneratePdf) },
...
fun CurrentStockScreen(
    state: CurrentStockContract.State,
    processEvent: (CurrentStockContract.Event) -> Unit,
    ...
    Column(modifier = modifier.padding(16.dp).fillMaxSize()) {
        ...
        Button(
            onClick = { processEvent(CurrentStockContract.Event.ApplyFilters) },
...
@Composable
private fun StockTable(stockList: List<CurrentStockInfo>) {
...
private fun StockRow(info: CurrentStockInfo) {
    Row(
        ...
        Text(
            String.format("%.2f", info.quantity),
            modifier = Modifier.weight(1f),
            style = MaterialTheme.typography.bodyMedium,
            textAlign = TextAlign.End
        )
